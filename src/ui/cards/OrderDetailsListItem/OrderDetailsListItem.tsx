...
export const OrderDetailsListItem: React.FC<OrderDetailsListItemProps> = ({ type, value }) => {
  ...
  const getTitle = React.useCallback((type: OrderDetailsListItemProps['type']) => {
    switch (type) {
      case "mealCost": return t`meal_cost`
      case "cookTime": return t`cook_time`
      case "deliveryTime": return t`delivery_time`
      case "deliveryCost": return t`delivery_cost`
      case "totalCost": return t`total_cost`
      default: t`total_cost`
    }
  }, [type])

  const getIcon = React.useCallback((type: OrderDetailsListItemProps['type']) => {
  ...
  const getValue = React.useCallback((type: OrderDetailsListItemProps['type']) => {
    if (type.includes("Cost")) {
      return `${value.cost}`
    }
    else if (type === "deliveryTime") {
  ...
  return (
    <Box w="100%" padding="0 0.125rem">
      <Flex justify="space-between" align="center" w="100%">
        ...
        <Text fontSize="1.07rem" fontWeight={600} lineHeight="1.25rem">{getValue(type)}</Text>
      </Flex>
    </Box>
  );
};